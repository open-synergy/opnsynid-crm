...
class CRMLead(models.Model):
    ...
    def onchange_stage_change_constrain_template_id(self):
        obj_stage_change_cons_tmpl = \
            self.env["crm.stage_change_constrain_template"]
    ...
    def _check_stage_constrain(self):
        for document in self:
            if document.stage_change_constrain_template_id:
                detail_ids = \
                    document.stage_change_constrain_template_id.detail_ids
                check_detail_ids = detail_ids.filtered(
                    lambda r: r.stage_id == document.stage_id)
                if check_detail_ids:
                    status_check_item_ids = \
                        check_detail_ids.status_check_item_ids
